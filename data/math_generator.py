...
def _sample_chain(cfg: Config, rng: np.random.Generator):
    steps = int(rng.integers(cfg.min_steps, cfg.max_steps + 1))
    ops = rng.integers(1, cfg.vocab_size, size=steps, dtype=np.int32)
    total = int(np.sum(ops))
    target = total % cfg.output_dim

    x = np.zeros(cfg.seq_len, dtype=np.int32)
    x[:steps] = ops
    y = np.zeros(cfg.seq_len, dtype=np.int32)
    y[0] = target
    return x, y


def _build_split(cfg: Config, n: int, rng: np.random.Generator) -> SplitArrays:
    inputs = np.zeros((n, cfg.seq_len), dtype=np.int32)
    labels = np.zeros((n, cfg.seq_len), dtype=np.int32)
    for i in range(n):
        x, y = _sample_chain(cfg, rng)
        inputs[i] = x
        labels[i] = y
...
def _save_split(cfg: Config, split: str, arrays: SplitArrays):
    ...
    np.save(os.path.join(split_dir, "all__inputs.npy"), arrays.inputs + 1)
    np.save(os.path.join(split_dir, "all__labels.npy"), arrays.labels + 1)
...
def parse_args() -> Config:
    ...
    p.add_argument("--output-dir", default="data/math-chains")
